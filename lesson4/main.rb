...
class Main
  ...
  def create_train
    ...
    print "Select train type: cargo, passenger: "
    type = gets.strip

    type == "cargo" ? trains.push(CargoTrain.new(number)) : trains.push(PassengerTrain.new(number))
  ...
  def trains_list_by_station
    ...
    station.trains_list do |train, index|
      puts "#{index+1}) #{train.train_number} - #{train.train_type}"
    end
  end
  ...
  def remove_station_from_route(route)
    if route.stations.count <= 2
      puts "You can't remove any station."
    else
