class Train
  ...
  attr_reader :speed, :number, :train_type, :carriages
  ...
  def add_carriage(carriage)
    if @speed == 0
      @carriages << carriage if carriage.carriage_type == @train_type
    else
  ...
  def remove_carriage(carriage)
    if @speed == 0
      @carriages.delete(carriage) if carriage.carriage_type == @train_type
    else
      puts 'Stop train before change carriage count.'
    end
  end
